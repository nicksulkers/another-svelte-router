...
export default {
	...
	_updateCurrentRouteOld: async function (url = location.pathname) {
		let _this = this;
		let paramsMap = new WeakMap();
		...
		async function nextRoute() {
			...
			if (matchedParamValues) {
				...
				matchedParamValues.shift();
				while (matchedParamValues.length) {
					params[paramNames.shift()] = matchedParamValues.shift();
				}
			}
			...
			async function nextMiddleware() {
				...
				if (typeof middleware === 'function' ? !(await Promise.resolve(middleware({params}))) : !middleware)
					return nextRoute();
		...
		await nextRoute();
		_this._trigger('afterRouteChange', [this._currentRoute]);
		return _this;
	},
